  ...
  <div class="container">
    <div class="row py-5">
      ...
      y mi animal favorito es la <?php print($arreglobjeto[animal]);?>.
        ...
        <hr>
        <p> Tengo información de los siguientes planetas de StarWars:</p>
        <ul>
          <?php
          for($i= 0; $i < count($csv); $i++){ //count=8

            if ($i==count($csv)-1) { // esto se hace para poner un punto al final
              print($csv[$i][name].'<strong>.</strong>');

            } else  if ($i == count($csv)-2){ //si i es distinto de 7 se imprime lo siguiente
              print($csv[$i][name].'<strong> y </strong>');

            } else { //si i es distinto de 7 se imprime lo siguiente
              print($csv[$i][name].'<strong>, </strong>');
